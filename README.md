...
- ClipTap 플레이어 패널을 진행바와 겹치지 않도록 더 위로 이동
- 플레이어 패널에 `전체 다운로드` 버튼 추가
- 로컬 헬퍼가 구간 다운로드와 전체 다운로드 모드를 모두 처리하도록 변경
- 플레이어 컨트롤 버튼 로고를 외부 PNG 파일로 교체 가능하게 변경
...
1. `cliptap-v1.1.2-chrome.zip` 압축 풀기
...
```bash
git commit -m "fix: move player panel and add full download"
```
