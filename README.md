...
- ClipTap 플레이어 패널에 반복 토글 아이콘 추가
- 반복이 켜져 있으면 선택한 시작~끝 구간을 자동으로 반복 재생
- 반복을 켜면 재생 헤드가 선택 구간 시작점으로 이동
...
1. `cliptap-v1.1.6-chrome.zip` 압축 풀기
...
7. 필요하면 제목 오른쪽의 반복 아이콘을 눌러 선택 구간 반복 재생
8. `구간 받기`
...
```txt
feat: add selected range loop playback
```

### Helper dependency notes

If the helper says `yt-dlp: NOT FOUND` even after `pip install yt-dlp`, v1.1.5 can also run yt-dlp through the current Python interpreter with `python -m yt_dlp`.

`pip install ffmpeg` does **not** install the real `ffmpeg.exe`. Install FFmpeg as a system command, or place `ffmpeg.exe` inside `helper/bin/ffmpeg.exe`.
